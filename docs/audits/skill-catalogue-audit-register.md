...
| `autonomous-lab` (Wk-C split) | **Gut to a thin autonomous *conductor*** for large missions: autonomous scoping + self-drive + durable cross-session state/recovery + hard STOP gate, **human-out-of-loop**. Delegates lifecycle execution to existing skills; sheds ~17K words (routing, decision-lifecycle, filesystem-memory, anti-placebo, workflow-patterns) — drop or move to owners. **Rename** (name pending — candidates: `autopilot`, `mission`, `unattended`). **Relocate its written state** (GOAL/STATE/ADR/QUEUE) out of the skill dir into a session-owned run location under `.agent-run/<mission-id>/`, consistent with how `session`/other runs store artifacts (via `bootstrap-lab.sh` + filesystem-memory rework). |
...
| Licensing (Wk-E, 8 human-calls) | **Retain-all + consolidate location** to `LICENSES/` + top-level `NOTICE` + `THIRD_PARTY_NOTICES.md` index — **except DROP**, all gated by an upstream-diff verification (flag back if substantial *expression* still copied): **Superpowers** (diagnose/tdd/skill-authoring), **Skill Optimizer** (skill-audit), and **Blader/humanizer** (natural-writing). **⚠ Blader caveat:** the repo's own `skills/natural-writing/NOTICE.md` states it "began as a modified distribution of humanizer" — this *contradicts* the owner's "not derived from there," so the diff is mandatory before removing a public-repo attribution. `caveman` `THIRD_PARTY_NOTICES` gap **DONE** (`4f2ecc3`). Retain grill-me/Pocock, BMAD, caveman. |
...
| `grill-me` `spec05_cases.yaml` | **BLOCKED — not local redundancy.** It is grill-me's slice of the harness-wide `spec05-skill-routing` eval consumed by `skills/orchestrate/evals/spec05_skill_evaluation.py` (hard read, no guard) and asserted by `tests/test_spec05_skill_alignment.py` (grill-me in `AFFECTED`, requires exactly positive/negative/adjacent/portability). 9 skills carry it. Merging/deleting for grill-me alone breaks the runner + test + cross-skill consistency. Owner decision pending: keep as-is (recommended) vs. scope a full spec05-harness removal across all 9 skills. |
