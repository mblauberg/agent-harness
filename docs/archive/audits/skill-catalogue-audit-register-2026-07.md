...
**Routing evidence follow-up — resolved by #135:** the catalogue restructure exposed that checked-in per-file hashes made routine skill changes invalidate an entire provider-bound bundle. The immutable earlier bundle is retained as historical evidence; the maintained lifecycle gate now validates current fixtures and executes adapter-absent workflows deterministically. Repository-owned portfolio routing evidence uses Git commit plus path provenance, while external provider receipt and result identities remain separately bound. No provider answer was edited or relabelled.
...
**CI green-up + Codex review dispositions (this branch):**
- **House-style gate — FIXED:** the #124 restructure shipped a new, stricter `style_lint.py` (adds `center` to the US-spelling list; not on `main`) alongside content that violated it; the superseded lifecycle gate was short-circuiting `check-harness` before the style gate ran, hiding it. All branch-introduced findings cleared: em dashes removed from `HARNESS.md`/`AGENTS.md`/`MAINTAINING.md` and five skill front doors; `human-out-of-loop`/`not just` recast; the `vercel-react-best-practices-MIT` link text reworded; README de-centred to drop the `align="center"` false positive.
- **Codex P2 (README authority) — FIXED:** the "What Provenant adds" bullet no longer says Provenant "approves work"; it "requires human approval before implementation starts", matching the lifecycle diagram and `HARNESS.md`.
- **Codex P2 (modern-screenshot URL) — FIXED:** the upstream source URL now lives in `THIRD_PARTY_NOTICES.md` (the prose legal index), not only in `ACKNOWLEDGEMENTS.md`.
- **Codex P3 (autopilot vs worktree envelope) — FIXED:** `skills/autopilot/SKILL.md` no longer tells agents source-repo implementation branches/worktrees need separate human authority; it points at the constitution's standing envelope and keeps deletion/integration/shared-branch pushes gated. `scripts/worktree.py` flags are the attestation mechanism and stay as-is.

**Resolved after this branch:**
- **#135:** direct milestone-codename retirement and commit-plus-path routing provenance landed on its child branch.
- **Codex P2 (release-gate trackedness), resolved through [PR #308](https://github.com/mblauberg/provenant/pull/308):** `public_release_check.scan_paths` now validates required legal files against tracked-set membership and covers the untracked-decoy case.
...
| `grill-me` milestone fixtures | **RESOLVED by issue #135.** The focused cases now live under the descriptive `lifecycle_cases.yaml` owner. The direct cutover also renamed runtime/configuration owners, amended ADR 0008 and separated immutable historical provider evidence from current deterministic gates. |
...
- **grill-me** — reach (c primary — loaded from inside `scope`'s workflow; a for direct "grill me"). Leanest body (187w), appropriate. Its `evals/lifecycle_cases.yaml` fixture is the focused lifecycle-composition and portability owner; `evals/trigger_cases.yaml` remains the general trigger boundary suite. Bare `LICENSE`, no NOTICE (provenance-gap pattern, see below + Workstream E).
