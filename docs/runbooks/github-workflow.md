...
Use this process for human- and agent-originated repository work. Project
...
| Awaiting human | Machine work is ready but a human decision or acceptance remains. |
...
1. Check the evidence, desired outcome, scope, acceptance evidence,
   dependencies, risk, authority and human gates.
...
3. Agents may triage only inside their granted authority. A spec, one-way-door
   choice, final acceptance or release decision moves to `Awaiting human`; an
   agent never infers it. Human-originated items follow the same evidence and
   scope checks.

## Execute and review

1. Set `In progress` when an owner starts the accepted scope.
2. Link a pull request with `Closes #N` only when merge leaves no human or
...
4. If machine gates pass but a human gate remains, set `Awaiting human`.
5. A human decision comment names the selected choice, artifact or exact head,
