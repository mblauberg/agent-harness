# Agent Fabric persistence and cutover

## Asynchronous lifecycle rotation persistence

the lifecycle-custody contract owns observable behaviour. `lifecycle_rotation_custodies` plus its append-only revisions and exact
head is the dedicated owner for rotate/compact provider and bridge effects, including a true chair rotation.
