...
Every current `provider_action_routes` row for an answer-bearing action stores
non-null `capability_snapshot_generation`, `capability_snapshot_digest`,
...
`host_identity_digest`, `requested_authority_profile_digest`,
...
`authority_compiler_version`, `expected_authority_profile_policy_version`,
`authority_profile_policy_version`, `deployed_route_admission_json`,
`deployed_route_admission_digest`, `route_policy_revision`, `harness_revision`,
`harness_digest`, `context_policy_revision`, `context_policy_digest`,
`permission_profile_digest`, `discovery_surface_evidence_id`,
`discovery_surface_evidence_revision` and `discovery_surface_digest`. Foreign
keys bind the exact adapter/generation/digest.
