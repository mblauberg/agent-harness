# Project Fabric Console acceptance and usability
...
- one current target binds the current fabric-sealed
...
- one-chair/topology-wave views for single-owner, explicit Fabric and opaque
...
- fresh implementation versus same-history crash-recovery and parent/child
   lifecycle independence;
- the exact per-arm display matrix, authorised substitution, separate
...
- `Not requested`, `Configured default`, `Inapplicable`, observed `None` and
...
- host-native negative cases for a second chair, embedded semantic gate,
   unpartitioned writer scope and double decomposition ownership;
- route-evidence expiry/baseline display and content-free operational spans;
- context pressure remaining independent from spend across unknown, stale and
   exact composite-audit-bound observations; and
- certifying provider/family/model actual-route proof, every observed route-
