...
def test_check_detects_and_install_reconciles_stale_managed_skill(tmp_path):
    ...
    assert "deliver=missing" in checked.stderr
...
def test_installer_preserves_existing_entries(tmp_path):
    ...
    checked = run(target, "--check")
    assert checked.returncode == 3
    assert "scope=noncanonical" in checked.stderr
...
def test_check_accepts_unmanaged_exact_link_as_compatible(tmp_path):
    ...
    assert '"state": "present"' in checked.stdout
...
def test_check_treats_only_exact_unmanaged_links_as_compatible(tmp_path):
    ...
    assert states == {"alpha": "present", "beta": "present"}
    assert not manifest_for(target).exists()
...
def test_cross_process_lock_prevents_disjoint_manifest_lost_update(tmp_path):
    ...
    beta_source = named_source(tmp_path / "beta-owner", ("alpha", "beta"))
    target = tmp_path / "installed"
    ready = tmp_path / "ready"
    release = tmp_path / "release"
