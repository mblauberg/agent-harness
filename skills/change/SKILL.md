---
name: change
description: Use when carrying an approved feature, bugfix, refactor, migration, or dependency upgrade through implementation, verification, independent review, bounded repair, documentation, and final human acceptance — especially "implement this autonomously", "take this spec to completion", or "review and fix until clean". Not for unclear requirements (scope), a known failure needing root cause only (diagnose), or sprawling run-until-STOP work (autonomous-lab).
---

# Change

Own an ordinary software change from an approved contract to a verified human
...
7. Run `${AGENTS_HOME:-$HOME/.agents}/skills/change/scripts/validate_run.py RUN.json`.
