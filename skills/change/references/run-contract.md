...
stability, a normal scope/change run reconciles provisional decisions, tests,
...
`${AGENTS_HOME:-$HOME/.agents}/skills/change/scripts/validate_run.py RUN.json`
validates the machine gate and permits
`phase: awaiting-human` with final approval pending.

`${AGENTS_HOME:-$HOME/.agents}/skills/change/scripts/validate_run.py --gate complete RUN.json`
