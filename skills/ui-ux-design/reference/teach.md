...
- **PRODUCT.md exists, DESIGN.md missing**: skip to Step 5 and offer to run `$ui-ux-design document` for DESIGN.md.
...
If teach was invoked as a setup blocker by another command, such as `$ui-ux-design craft landing page`, pause that command here. Complete teach, re-run the loader, then resume the original command with the freshly loaded context. For craft, resume into shape next; teach creates project context, but it is not a substitute for the task-specific shape interview and confirmed design brief.
...
Write to `PROJECT_ROOT/PRODUCT.md`. If `.impeccable.md` existed, use its content
as input but leave the legacy file in place; migration is a separate authorised
file edit.

## Step 5: Decide on DESIGN.md

Offer `$ui-ux-design document` either way. Two paths:
...
If the user agrees, delegate to `$ui-ux-design document` (it auto-detects scan vs seed). Load its reference and follow that flow.

If the user prefers to skip, mention they can run `$ui-ux-design document` any time later.
...
If teach was invoked as a blocker by another impeccable command (e.g. the user ran `$ui-ux-design polish` with no PRODUCT.md), resume that original task now with the fresh context.
