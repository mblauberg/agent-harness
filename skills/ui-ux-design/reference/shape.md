...
`scope` owns the approved specification, authority and user gate. Shape is its
...
**Output**: A design brief that can be handed off to $ui-ux-design craft, or directly to $ui-ux-design for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Once confirmed, the brief is complete. The user can now hand it to $ui-ux-design, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use $ui-ux-design craft instead, which runs this command internally.)
