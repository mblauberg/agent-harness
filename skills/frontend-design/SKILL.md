...
If the output is already in this session's conversation history, don't re-run. Exceptions requiring a fresh load: you just ran `$frontend-design teach` or `$frontend-design document` (they rewrite the files), or the user manually edited one.

`$frontend-design live` already warms context via `live.mjs`. If you've run `live.mjs`, don't also run `load-context.mjs` this session.

If PRODUCT.md is missing, empty, or placeholder (`[TODO]` markers, <200 chars): run `$frontend-design teach`, then resume the user's original task with the fresh context. If the original task was `$frontend-design craft`, resume into `$frontend-design shape` before any implementation work.

If DESIGN.md is missing: nudge once per session (*"Run `$frontend-design document` for more on-brand output"*), then proceed.
...
If PRODUCT.md lacks the `register` field (legacy), infer it once from its "Users" and "Product Purpose" sections, then cache the inferred value for the session. Suggest the user run `$frontend-design teach` to add the field explicitly.
...
[references/](references/) holds concrete lookup CSVs (67 UI styles, 96 color palettes, 57 font pairings, 25 chart types) — consult them for candidate options during `colorize`, `typeset`, `craft`, or `shape`, then apply the shared design laws above to pick and adapt, not copy verbatim.
...
Setup (context gathering, register) is already loaded by then; sub-commands don't re-invoke `$frontend-design`.
...
**Pin** creates a standalone shortcut so `$<command>` invokes `$frontend-design <command>` directly. **Unpin** removes it. The script writes to every harness directory present in the project.
