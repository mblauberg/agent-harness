 ...
 *
 * `pin audit` creates a lightweight /audit skill that redirects to /frontend-design audit.
...
/**
 * Find project harness skill directories. The canonical skill may be global;
 * accept local frontend-design or legacy impeccable installations too.
 */
function findHarnessDirs(projectRoot) {
  const dirs = [];
  for (const harness of HARNESS_DIRS) {
    const skillsDir = join(projectRoot, harness, 'skills');
    const frontendDir = join(skillsDir, 'frontend-design');
    const legacyDir = join(skillsDir, 'impeccable');
    if (existsSync(skillsDir) || existsSync(frontendDir) || existsSync(legacyDir) || existsSync(join(skillsDir, 'i-impeccable'))) {
      dirs.push(skillsDir);
    }
  }
  return dirs;
...
function generatePinnedSkill(command, metadata) {
  const desc = metadata[command]?.description || `Shortcut for /frontend-design ${command}.`;
...
This is a pinned shortcut for \`{{command_prefix}}frontend-design ${command}\`.

Invoke {{command_prefix}}frontend-design ${command}, passing along any arguments provided here, and follow its instructions.
`;
}
...
function pin(command, projectRoot) {
  ...
  if (harnessDirs.length === 0) {
    console.log('No project harness skill directories found.');
    return false;
  }
...
function unpin(command, projectRoot) {
  ...
  if (removed > 0) {
    ...
    console.log(`Use /frontend-design ${command} to access it.`);
  } else {
