...
- The user just ran `$frontend-design teach` and needs the visual side documented.
...
Decide by scanning first (Scan mode Step 1). If the scan finds no tokens, no component files, and no rendered site, offer seed mode; don't silently switch. `$frontend-design document --seed` forces seed mode regardless of code presence.
...
4. **Refresh the session cache.** Run `node "${AGENTS_HOME:-$HOME/.agents}/skills/frontend-design/scripts/load-context.mjs"` one final time so the newly-written DESIGN.md lands in conversation. Subsequent commands in this session will use the fresh version automatically without re-reading.
...
Before interviewing: "There's no existing visual system to scan. I'll ask five quick questions to seed a starter DESIGN.md. You can re-run `$frontend-design document` once there's code, to capture the real tokens and components. OK?"
...
```markdown
<!-- SEED: re-run $frontend-design document once there's code to capture the actual tokens and components. -->
```
...
2. Tell the user: "Re-run `$frontend-design document` once you have some code. That pass will extract real tokens and generate the sidecar."
3. Run `node "${AGENTS_HOME:-$HOME/.agents}/skills/frontend-design/scripts/load-context.mjs"` once so the seed lands in conversation for the rest of the session.
