...
2. **Compute the slug**:
   ```bash
   node "${AGENTS_HOME:-$HOME/.agents}/skills/frontend-design/scripts/critique-storage.mjs" slug "<resolved-path-or-url>"
   ```
...
CLI scan:
```bash
node "${AGENTS_HOME:-$HOME/.agents}/skills/frontend-design/scripts/detect.mjs" --json [--fast] [target]
```
...
4. If mutation is available, start `node "${AGENTS_HOME:-$HOME/.agents}/skills/frontend-design/scripts/live-server.mjs" --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
Codex final-answer note: `$frontend-design critique` produces a report artifact, so the final chat response should intentionally exceed the usual concise close-out style. Do not title the final response "Critique Summary" unless the user explicitly asked for a summary.
...
- **Suggested command**: Which command could address this (from: $frontend-design adapt, $frontend-design animate, $frontend-design audit, $frontend-design bolder, $frontend-design clarify, $frontend-design colorize, $frontend-design critique, $frontend-design delight, $frontend-design distill, $frontend-design document, $frontend-design harden, $frontend-design layout, $frontend-design onboard, $frontend-design optimize, $frontend-design overdrive, $frontend-design polish, $frontend-design quieter, $frontend-design shape, $frontend-design typeset)
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node "${AGENTS_HOME:-$HOME/.agents}/skills/frontend-design/scripts/critique-storage.mjs" write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node "${AGENTS_HOME:-$HOME/.agents}/skills/frontend-design/scripts/critique-storage.mjs" trend <slug> 5
   ```
...
- Only recommend commands from: $frontend-design adapt, $frontend-design animate, $frontend-design audit, $frontend-design bolder, $frontend-design clarify, $frontend-design colorize, $frontend-design critique, $frontend-design delight, $frontend-design distill, $frontend-design document, $frontend-design harden, $frontend-design layout, $frontend-design onboard, $frontend-design optimize, $frontend-design overdrive, $frontend-design polish, $frontend-design quieter, $frontend-design shape, $frontend-design typeset
...
- End with `$frontend-design polish` as the final step if any fixes were recommended

After presenting the summary, tell the user:

> You can ask me to run these one at a time, all at once, or in any order you prefer.
>
> Re-run `$frontend-design critique` after fixes to see your score improve.
