...
Once confirmed, the brief is complete. The user can now hand it to $frontend-design, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use $frontend-design craft instead, which runs this command internally.)
