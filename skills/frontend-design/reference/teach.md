...
- **PRODUCT.md exists, DESIGN.md missing**: skip to Step 5 and offer to run `$frontend-design document` for DESIGN.md.
...
If teach was invoked as a setup blocker by another command, such as `$frontend-design craft landing page`, pause that command here. Complete teach, re-run the loader, then resume the original command with the freshly loaded context. For craft, resume into shape next; teach creates project context, but it is not a substitute for the task-specific shape interview and confirmed design brief.
...
Offer `$frontend-design document` either way. Two paths:
...
If the user agrees, delegate to `$frontend-design document` (it auto-detects scan vs seed). Load its reference and follow that flow.

If the user prefers to skip, mention they can run `$frontend-design document` any time later.
