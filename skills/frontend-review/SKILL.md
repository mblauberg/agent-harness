...
description: "Use for source- or browser-read-only review of frontend UX, visuals, accessibility, responsiveness, and interaction states. Not for design edits or pure React profiling; use frontend-design or react-performance."
---
...
submit/send/purchase/delete/account actions are outside review authority.
...
current WCAG/Lighthouse facts and `react-performance` for measured React claims.
Those overlays supply evidence; this skill owns the UI finding contract.
...
For each genuine issue give severity, exact surface evidence, user impact,
mechanism, smallest fix route and validation step. Report unsupported or
sensitive surfaces and retained artifacts. Return `clean within tested
coverage` when nothing survives, followed by material untested lanes.

Fixes route to `frontend-design` under `implement`; re-review remains
independent and source-read-only.
