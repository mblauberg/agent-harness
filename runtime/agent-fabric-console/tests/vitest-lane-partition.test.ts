 ...
 * case-insensitive *substring* filter over that list (see `filterFiles` in
 * vitest's cli-api chunk) -- not an anchored directory glob. A hand-rolled
 * reimplementation of that filter would either miss this substring
 * behaviour (understating what a dedicated lane actually runs) or have to
 * track Vitest's exact, undocumented algorithm forever. Shelling out to
 * `vitest list --json` sidesteps both problems: it is always exactly what
 * CI's `npm run test*` scripts would execute.
 */
...
describe("console vitest lane partition", () => {
  it(
