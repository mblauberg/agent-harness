...
describe("S2a guards: lifecycle continuations get their own map and drain", () => {
  ...
  // (fabric.ts:9245, "a still-pending standard-key generic predecessor prevents the lifecycle
  // continuation's provider effect until the predecessor settles") and CLOSE-3 ("close() started
  // while a lifecycle continuation is waiting on its predecessor drains correctly") CANNOT be
  // driven live from any externally reachable call sequence in the current implementation. This is
  // a stronger finding than the existing CLOSE-3 skip
  ...
  //
  // The predecessor lookup (fabric.ts:9241-9243) reads #ownedProviderActions keyed by
  // `runId\0adapterId\0callerActionId`, where callerActionId is always the action_id of the
  // CALLING agent's currently-active provider_session_turn_leases row (fabric.ts:8090,8111 --
  // the "active lifecycle caller turn" query, an INNER JOIN so acceptRotation cannot proceed
  // without one). Turn leases are inserted in exactly one place, `prepareTurnAction`
  // (provider-session-coordinator.ts:403), invoked only for a "send_turn" dispatch
  // (fabric.ts:6183-6208). #ownedProviderActions standard-key entries, meanwhile, are written in
  // exactly one place, #enqueueDeferredProviderAction (fabric.ts:1507-1526), invoked only from the
  // ephemeral, task-bound, answer-bearing "spawn" path (fabric.ts:6112-6128, the ONLY call site of
  // `deferCompletion: true` in fabric.ts -- confirmed by exhaustive grep). A single
  // (runId, adapterId, actionId) triple is admitted under exactly one operation, so an action can
  // never be both the caller's active turn lease AND a live #ownedProviderActions entry: "send_turn"
  // actions (the only source of callerActionId) are dispatched through the direct, non-deferred
  // path (fabric.ts:6272) and never touch #enqueueDeferredProviderAction at all. KEY-1 above
  // independently confirms the generic (ephemeral-spawn) and lifecycle (send_turn-triggered
  // rotate) action families are simultaneously drivable but never share an actionId, which is the
  // same disjunction that makes the predecessor lookup permanently miss. This holds regardless of
  // process topology (verified against both the real daemon subprocess used by KEY-1 above, and,
  // separately, the retained fixture's in-process `fault` variant, which also cannot manufacture
  // the combination -- and which additionally does not forward the spawn-barrier environment to
  // its adapter, a second, independent reason it cannot drive this scenario). Kept .skip (never
  // .fails) so it cannot green the base; do not delete these without either wiring a new caller
  // shape that lets a send_turn's own action ride the deferred/#ownedProviderActions path, or
  // downgrading the predecessor-await claim in the S2 plan's preservation checklist (docs section
  // 4, "Predecessor ordering" row) to reflect that it is currently unreachable rather than merely
  // untested.
