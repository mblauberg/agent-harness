...
function assertSchema(database: Database.Database): void {
  ...
  const expected: Readonly<Record<string, readonly string[]>> = {
    authority_metadata: ["singleton", "schema_version", "authority_id"],
    admitted_scopes: ["project_session_id", "run_id", "project_id", "scope_json", "scope_attestation"],
    receipts: ["project_session_id", "run_id", "authority_sequence", "kind", "agent_id", "owner_ref_digest", "owner_revision", "intent_digest", "subject_json", "receipt_json", "receipt_digest"],
    scope_snapshots: ["checkpoint_digest", "project_session_id", "run_id", "max_authority_sequence", "checkpoint_json"],
    namespace_snapshots: ["checkpoint_digest", "project_id", "checkpoint_json"],
    namespace_snapshot_members: ["checkpoint_digest", "member_order", "scope_checkpoint_digest"],
  };
  const tables = schemaTableNames(database);
  if (canonicalJson(tables) !== canonicalJson(Object.keys(expected).sort())) {
    throw new Error("lifecycle receipt schema mismatch");
  }
  for (const [table, columns] of Object.entries(expected)) {
    const actual = database.prepare(`SELECT name FROM pragma_table_info(?) ORDER BY cid`).all(table)
      .map((entry) => String((entry as { name: unknown }).name));
    if (canonicalJson(actual) !== canonicalJson(columns)) throw new Error("lifecycle receipt schema mismatch");
  }
}

function schemaTableNames(database: Database.Database): string[] {
  return database.prepare(`
    SELECT name FROM sqlite_schema
     WHERE type='table' AND name NOT LIKE 'sqlite_%'
     ORDER BY name
  `).all().map((entry) => String((entry as { name: unknown }).name));
}
