...
export class Fabric {
  ...
  bootstrapCurrentMcpSeat(input: BootstrapMcpSeatInput): BootstrapMcpSeatResult {
    const canonicalRoot = canonicalWorkspaceRoot(input.canonicalRoot);
    if (canonicalRoot !== input.canonicalRoot || !this.#workspaceRoots.includes(canonicalRoot)) {
    ...
    if (!Number.isFinite(requestedExpiry) || requestedExpiry <= this.#clock()) {
    ...
    return this.#database.transaction((): BootstrapMcpSeatResult => {
      ...
      if (existingSession.length === 0) {
        const authority = normaliseAuthority({
          schemaVersion: 2,
          approval: {
        ...
        this.#database.prepare("INSERT INTO mailbox_state(run_id,recipient_id) VALUES (?,?)").run(runId, chairAgentId);
        const initialCapability = capabilityToken(this.#capabilityKey, runId, chairAgentId, 1);
        this.#database.prepare(`
          INSERT INTO capabilities(token_hash,run_id,agent_id,principal_generation,expires_at)
          VALUES (?,?,?,1,?)
        `).run(sha256(initialCapability), runId, chairAgentId, requestedExpiry);
        this.#database.prepare(`
        ...
        this.#database.prepare("INSERT INTO run_metadata(run_id,execution_profile) VALUES (?,'headless')").run(runId);
        this.#database.prepare(`
      ...
      if (isRow(active)) {
        ...
        if (member !== undefined) {
          ...
          const credentials = bindings.map((binding) => ({
            ...binding,
            capability: `afc_${createHmac("sha256", this.#capabilityKey)
              .update(canonicalJson({
                kind: "current-mcp-seat",
                canonicalRoot,
                projectSessionId,
      ...
      if (requestedAgentId !== currentChairAgentId && this.#database.prepare(
        "SELECT 1 FROM agents WHERE run_id=? AND agent_id=?",
      ).get(runId, requestedAgentId) === undefined) {
        const chairAuthority = rowOrNotFound(this.#database.prepare(
        ...
        this.#database.prepare("INSERT INTO mailbox_state(run_id,recipient_id) VALUES (?,?)").run(runId, requestedAgentId);
        const token = capabilityToken(this.#capabilityKey, runId, requestedAgentId, 1);
        this.#database.prepare(`
          INSERT INTO capabilities(token_hash,run_id,agent_id,principal_generation,expires_at)
          VALUES (?,?,?,1,?)
        `).run(sha256(token), runId, requestedAgentId, requestedExpiry);
      }

      const bindings = this.#database.prepare(`
        SELECT CASE WHEN agent_id=? THEN ? ELSE ? END AS seat,agent_id AS agentId,1 AS expectedPrincipalGeneration
          FROM agents WHERE run_id=? ORDER BY seat
      `).all(currentChairAgentId, chairSeat, chairSeat === "codex" ? "claude" : "codex", runId) as CurrentMcpSeatBindingInput["bindings"];
      const expiresAt = isRow(active)
      ...
      const bound = this.bindCurrentMcpSeats({
        canonicalRoot,
